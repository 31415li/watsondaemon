...
"--orgid=<org ID> --typeid=<type ID> --deviceid=<device ID> --token=<auth token>";
...
static const struct poptOption options[] = {
    {"orgid",    '\0', POPT_ARG_STRING, NULL, OPT_ORGID,    "IoTP organization ID", "<org ID>"    },
    {"typeid",   '\0', POPT_ARG_STRING, NULL, OPT_TYPEID,   "IoTP device type ID",  "<type ID>"   },
    {"deviceid", '\0', POPT_ARG_STRING, NULL, OPT_DEVICEID, "IoTP device ID",       "<device ID>" },
    {"token",    '\0', POPT_ARG_STRING, NULL, OPT_TOKEN,    "IoTP auth token",      "<auth token>"},
    POPT_AUTOHELP
    POPT_TABLEEND
};
